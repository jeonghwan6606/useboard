	      ...
	      <h1>게시글 수정</h1>
			...
			<% 	
		      ...
		      <article style=" text-align:right;">
			      ...
			      <a  type="button" class="btn btn-dark" href="<%=request.getContextPath()%>/boardOne2.jsp?boardNo=<%=bO.getBoardNo()%>" role="button">취소</a>
		      </article>
		      
	      	<table class="table table-hover">
	      		<tr>
	      			<th class="table-info">제목</th>
	      			<td><input type="text" name="boardTitle" value="<%=bO.getBoardTitle()%>"></td>
	      			<td colspan="4"></td>
	      		</tr>
	      		<tr>
	      			<th class="table-info">번호</th>
	      			<td><input type= "text" value="<%=bO.getBoardNo()%>" name="boardNo" readonly="readonly"></td>
	      				...
	      				<select name="localName">
					      	    ...
					      	    <%
									for(HashMap<String, Object> l : localList) { 
											if(bO.getLocalName().equals(l.get("localName"))){
									
								%>
										<option  value="<%=(String)l.get("localName")%>" selected><%=(String)l.get("localName")%></option>
								<% 
								   			}else{
								%>   
	      		...
	      		<tr>
	      			...
	      			<td colspan="5"><textarea rows="10" cols="90" name="boardContent"><%=bO.getBoardContent()%></textarea></td>
	      		</tr>
	      	</table> 	
	      	</form>
	   </div>
